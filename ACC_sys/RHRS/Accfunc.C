...
void Accfunc(){
	...
	outfile1<<th_bin[ii]<<","<<acc_norm<<","<<acc_norm_err<<endl;
     ...
     gACC_v->Draw("AP");
     gACC_v->SetTitle("RHRS acceptance function;ev.Th (deg)");
     ...
     tex.DrawLatexNDC(0.1,0.4,"up plane cut");
     tex.DrawLatexNDC(0.1,0.35,"down plane cut");
     ...
     hinc_v->SetTitle("RHRS incident angle distribution;ev.Th (deg);");

     c2->cd(2);
     hacc_v->Draw();
     hacc_v->SetLineColor(4);
     hacc_v->SetTitle(" RHRS accepted angle distribution;ev.Th (deg);");
     ...
     gACC_v_norm->Draw("AP");
     gACC_v_norm->SetTitle("RHRS acceptance function (normalized);ev.Th (deg)");
     ...
     tex1.DrawLatexNDC(0.1,0.4,"up plane cut");
     tex1.DrawLatexNDC(0.1,0.35,"down plane cut");
