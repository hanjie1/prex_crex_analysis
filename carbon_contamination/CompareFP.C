...
// set1
   ...
   Double_t z_vdc_fp = 1.37;
   Double_t delx_fp = -0.01;
   Double_t dely_fp = -0.021;
...
/*   Double_t delx = -0.004;
   ...
   Double_t z_vdc_fp = 1.68;
   Double_t delx_fp = -0.01;
   Double_t dely_fp = -0.022;
*/
...
//======== focal plane ===========//
   ...
   if(islogy){
	...
	trx_min=-0.15; trx_max=0.1; try_min=-0.08; try_max=0.04;
	trth_min=-0.2; trth_max=0.05; trph_min=-0.08; trph_max=0.08;
   }

   TH1F *htrx_vdc = new TH1F("htrx_vdc","L.tr.x distribution",100,trx_min,trx_max);
   TH1F *htrx_fp = new TH1F("htrx_fp","L.tr.x at fp distribution",100,trx_min,trx_max);
   ...
   TH1F *htry_vdc = new TH1F("htry_vdc","L.tr.y distribution",100,try_min,try_max);
   TH1F *htry_fp = new TH1F("htry_fp","L.tr.y at fp distribution",100,try_min,try_max);
   ...
   c1->Divide(2,2);
   c1->cd(1);
   T_data->Draw("L.tr.x>>htrx_vdc",TRK);
   T_MC->Draw(Form("x_vdc_tr+%f*th_vdc_tr+%f>>htrx_MC_vdc",delz,delx),MCcut*"rate","same");
   ...
   if(h1_max>h2_max){ 
      ...
      htrx_vdc->SetTitle(Form("%s VDC plane X; x;",target.Data()));
   }
   else{
      htrx_MC_vdc->Draw("HIST");
      htrx_vdc->Draw("HIST same");
      htrx_MC_vdc->SetTitle(Form("LHRS %s VDC plane X; x;",target.Data()));
   }
   ...
   c1->cd(2);
   T_data->Draw("L.tr.y>>htry_vdc",TRK);
   T_MC->Draw(Form("y_vdc_tr+%f*ph_vdc_tr+%f>>htry_MC_vdc",delz,dely),MCcut*"rate","same");
   ...
   if(h1_max>h2_max){ 
      ...
      htry_vdc->SetTitle("VDC plane Y; y;");
   }
   else{
      htry_MC_vdc->Draw("HIST");
      htry_vdc->Draw("HIST same");
      htry_MC_vdc->SetTitle("VDC plane Y; y;");
   }
   ...
   c1->cd(3);
   T_data->Draw(Form("L.tr.x+%f*L.tr.th>>htrx_fp",z_vdc_fp),TRK);
   T_MC->Draw(Form("x_vdc_tr+%f*th_vdc_tr+%f>>htrx_MC_fp",z_vdc_fp+delz,delx_fp),MCcut*"rate","same");

   Double_t ntrx_fp = htrx_fp->Integral();
   Double_t ntrx_mc_fp = htrx_MC_fp->Integral();
   htrx_MC_fp->Scale(ntrx_fp/ntrx_mc_fp);

   h1_max = htrx_fp->GetBinContent(htrx_fp->GetMaximumBin());
   h2_max = htrx_MC_fp->GetBinContent(htrx_MC_fp->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      htrx_fp->Draw("HIST");
      htrx_MC_fp->Draw("HIST same");
      htrx_fp->SetTitle(Form("%s detector plane X; x;",target.Data()));
   }
   else{
      htrx_MC_fp->Draw("HIST");
      htrx_fp->Draw("HIST same");
      htrx_MC_fp->SetTitle(Form("LHRS %s detector plane X; x;",target.Data()));
   }
   htrx_fp->SetLineColor(6);
   htrx_fp->SetLineWidth(2);
   htrx_MC_fp->SetLineColor(kAzure+1);
   htrx_MC_fp->SetLineWidth(2);
   if(islogy) gPad->SetLogy();

   TLegend *leg11 = new TLegend(0.15,0.6,0.35,0.8);
   leg11->AddEntry(htrx_fp,"data fp","L");
   leg11->AddEntry(htrx_MC_fp,"MC fp","L");
   leg11->Draw();

   c1->cd(4);
   T_data->Draw(Form("L.tr.y+%f*L.tr.ph>>htry_fp",z_vdc_fp),TRK);
   T_MC->Draw(Form("y_vdc_tr+%f*ph_vdc_tr+%f>>htry_MC_fp",z_vdc_fp+delz,dely_fp),MCcut*"rate","same");

   Double_t ntry_fp = htry_fp->Integral();
   Double_t ntry_mc_fp = htry_MC_fp->Integral();
   htry_MC_fp->Scale(ntry_fp/ntry_mc_fp);

   h1_max = htry_fp->GetBinContent(htry_fp->GetMaximumBin());
   h2_max = htry_MC_fp->GetBinContent(htry_MC_fp->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      htry_fp->Draw("HIST");
      htry_MC_fp->Draw("HIST same");
      htry_fp->SetTitle("detector plane Y; y;");
   }
   else{
      htry_MC_fp->Draw("HIST");
      htry_fp->Draw("HIST same");
      htry_MC_fp->SetTitle("detector plane Y; y;");
   }
   htry_fp->SetLineColor(6);
   htry_fp->SetLineWidth(2);
   htry_MC_fp->SetLineColor(kAzure+1);
   ...
   if(h1_max>h2_max){ 
      ...
      htrth->SetTitle("VDC plane Theta; theta;");
   }
   else{
      htrth_MC_vdc->Draw("HIST");
      htrth_MC_fp->Draw("HIST same");
      htrth->Draw("HIST same");
      htrth_MC_vdc->SetTitle("VDC plane Theta; theta;");
   }
   ...
   TLegend *leg2 = new TLegend(0.15,0.6,0.35,0.8);
   leg2->AddEntry(htrth,"data vdc","L");
   leg2->AddEntry(htrth_MC_vdc,"MC vdc","L");
   leg2->AddEntry(htrth_MC_fp,"MC fp","L");
   leg2->Draw();
   ...
   if(h1_max>h2_max){ 
      ...
      htrph->SetTitle("VDC plane Phi; phi;");
   }
   else{
      htrph_MC_vdc->Draw("HIST");
      htrph_MC_fp->Draw("HIST same");
      htrph->Draw("HIST same");
      htrph_MC_vdc->SetTitle("VDC plane Phi; phi;");
   }
...
//   c1->Print(Form("outfiles/%s_FP_LHRS.pdf",tg.Data()));
}
