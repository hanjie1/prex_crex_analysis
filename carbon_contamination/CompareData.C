...
//======== target ===========//
   ...
   T_MC->Draw("th_tg_tr>>htgth_MC",MCcut*"rate","same");

   Double_t ntgth = htgth->Integral();
   Double_t ntgth_mc = htgth_MC->Integral();
   htgth_MC->Scale(ntgth/ntgth_mc);
   
   h1_max = htgth->GetBinContent(htgth->GetMaximumBin());
   h2_max = htgth_MC->GetBinContent(htgth_MC->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      htgth->Draw("HIST");
      htgth_MC->Draw("HIST same");
      htgth->SetTitle(Form("%s target Theta; theta;",target.Data()));
   }
   else{
      htgth_MC->Draw("HIST");
      htgth->Draw("HIST same");
      htgth_MC->SetTitle(Form("%s target Theta; theta;",target.Data()));
   }
   ...
   leg2->AddEntry(htgth_MC,"MC","L");
   leg2->Draw();

   c2->cd(2);
   T_data->Draw("L.gold.ph>>htgph",TRK);
   T_MC->Draw("ph_tg_tr>>htgph_MC",MCcut*"rate","same");

   Double_t ntgph = htgph->Integral();
   Double_t ntgph_mc = htgph_MC->Integral();
   htgph_MC->Scale(ntgph/ntgph_mc);
   
   h1_max = htgph->GetBinContent(htgph->GetMaximumBin());
   h2_max = htgph_MC->GetBinContent(htgph_MC->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      htgph->Draw("HIST");
      htgph_MC->Draw("HIST same");
      htgph->SetTitle("target Phi; phi;");
   }
   else{
      htgph_MC->Draw("HIST");
      htgph->Draw("HIST same");
      htgph_MC->SetTitle("target Phi; phi;");
   }
   ...
   else { tgdp_min = -0.02; tgdp_max = 0.; }

   TH1F *htgdp = new TH1F("htgdp","L.gold.dp distgibution",100,tgdp_min,tgdp_max);
   TH1F *htgdp_MC = new TH1F("htgdp_MC","dp_tg_tr distgibution",100,tgdp_min,tgdp_max);

   c2->cd(3);
   T_data->Draw("L.gold.dp>>htgdp",TRK);
   T_MC->Draw("(p_tg_tr-950.0)/950.0>>htgdp_MC",MCcut*"rate","same");
   
   Double_t ntgdp = htgdp->Integral();
   Double_t ntgdp_mc = htgdp_MC->Integral();
   htgdp_MC->Scale(ntgdp/ntgdp_mc);
   
   h1_max = htgdp->GetBinContent(htgdp->GetMaximumBin());
   h2_max = htgdp_MC->GetBinContent(htgdp_MC->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      htgdp->Draw("HIST");
      htgdp_MC->Draw("HIST same");
      htgdp->SetTitle("target dp; dp;");
   }
   else{
      htgdp_MC->Draw("HIST");
      htgdp->Draw("HIST same");
      htgdp_MC->SetTitle("target dp; dp;");
   }
   htgdp->SetLineColor(2);
   htgdp->SetLineWidth(2);
   ...
   TH1F *hq2 = new TH1F("hq2","Q2 distgibution",100,q2_min,q2_max);
   TH1F *hq2_MC = new TH1F("hq2_MC","MC Q2 distgibution",100,q2_min,q2_max);

   TCanvas *c3 = new TCanvas("c3","c3",1500,1500);
   T_data->Draw("EK_L.Q2>>hq2",TRK);
   T_MC->Draw("ev.Q2>>hq2_MC",MCcut*"rate","same");
   
   Double_t nq2 = hq2->Integral();
   Double_t nq2_mc = hq2_MC->Integral();
   hq2_MC->Scale(nq2/nq2_mc);
   
   h1_max = hq2->GetBinContent(hq2->GetMaximumBin());
   h2_max = hq2_MC->GetBinContent(hq2_MC->GetMaximumBin());
  
   if(h1_max>h2_max){ 
      hq2->Draw("HIST");
      hq2_MC->Draw("HIST same");
      hq2->SetTitle("Q2 distribution; Q2;");
   }
   else{
      hq2_MC->Draw("HIST");
      hq2->Draw("HIST same");
      hq2_MC->SetTitle("Q2 distribution; Q2;");
   }
   hq2->SetLineColor(2);
   hq2->SetLineWidth(2);
   ...
   leg3->AddEntry(hq2_MC,"MC","L");
   leg3->Draw();

   TString out1, out2, out3;
   if(islogy){
     out1 = Form("outfiles/%s_log.pdf[",tg.Data());
     out2 = Form("outfiles/%s_log.pdf",tg.Data());
     out3 = Form("outfiles/%s_log.pdf]",tg.Data());
   }
   else{
     out1 = Form("outfiles/%s.pdf[",tg.Data());
     out2 = Form("outfiles/%s.pdf",tg.Data());
     out3 = Form("outfiles/%s.pdf]",tg.Data());
   }
